// Recurrentes: gastos automáticos + ingresos + cuentas por cobrar
const { useState: useStateRc } = React;
function Recurring({ recurring, setRecurring, income, receivables }) {
  const { Card, Badge, IconBtn, CatChip } = window.UI;
  const { Icon, fmtCLP, MES } = window.Helpers;
  const today = window.MOCK.TODAY;
  const monthLabel = `${MES[today.getMonth()]} ${today.getFullYear()}`;

  const [tab, setTab] = useStateRc("expenses"); // expenses | income | receivables

  // Cálculos
  const activeRec = recurring.filter(r => r.active);
  const totalRecurMonth = activeRec.reduce((s,r) => s+r.amount, 0);
  const chargedThisMonth = activeRec.filter(r => r.lastChargedMonth === `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,"0")}`);
  const pendingThisMonth = activeRec.filter(r => r.lastChargedMonth !== `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,"0")}`);
      ...
      <Card padding="p-0" className="overflow-hidden">
        ...
        {tab === "expenses" && (
          <ul className="divide-y divide-[var(--line)]">
            {recurring.map(r => {
              const cat = window.MOCK.CATEGORIES.find(c => c.id === r.category);
              const bank = window.MOCK.BANKS.find(b => b.id === r.bank);
              const monthStr = `${today.getFullYear()}-${String(today.getMonth()+1).padStart(2,"0")}`;
                ...
                <li key={r.id} className={`px-5 py-3.5 flex items-center gap-3 ${!r.active ? "opacity-55" : ""}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="mt-1 text-[11.5px] text-[var(--muted)] flex items-center gap-1.5 flex-wrap">
                      ...
                      <span>·</span>
                      <span>{r.method === "transfer" ? "Transferencia" : (r.type === "credito" ? "Crédito" : "Débito")}</span>
                    </div>
                  </div>
