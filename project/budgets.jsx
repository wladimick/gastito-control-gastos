...
function Budgets({ expenses, budgets, setBudgets }) {
  ...
  const rows = window.MOCK.CATEGORIES.map(c => {
  ...
  const dayRatio = dayNow / daysIn; // qué tanto del mes ha pasado

  const overCount = rows.filter(r => r.state === "over").length;
  const warnCount = rows.filter(r => r.state === "warn").length;

  const [editingId, setEditingId] = useStateBd(null);
  const [draft, setDraft] = useStateBd("");
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1.4fr_1fr] gap-6">
          <div>
            ...
            <div className="mt-3 flex items-center gap-2 flex-wrap text-[12.5px]">
              <Badge tone={totalPct >= 100 ? "warn" : (totalPct >= 80 ? "warn" : "ok")}>
                {totalPct.toFixed(0)}% usado
              </Badge>
              <span className="text-[var(--muted)]">·</span>
              <span className={remaining < 0 ? "text-[var(--amber-ink)]" : "text-[var(--ink-2)]"}>
                {remaining >= 0 ? "Quedan " : "Pasaste por "}<span className="font-mono">{fmtCLP(Math.abs(remaining))}</span>
              </span>
              <span className="text-[var(--muted)]">·</span>
              <span className="text-[var(--muted)]">Día {dayNow} de {daysIn}</span>
            </div>

            {/* Barra con marca de ritmo del mes */}
            <div className="mt-5 h-2 rounded-full bg-[var(--line)] overflow-hidden relative">
              ...
              <div className="absolute top-[-4px] bottom-[-4px] w-[2px] bg-[var(--ink)]"
                   style={{ left: (dayRatio*100) + "%" }} title={`Ritmo esperado: ${(dayRatio*100).toFixed(0)}%`}/>
            </div>
