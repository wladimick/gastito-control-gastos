// ExpensesList con filtros, búsqueda y acciones
const { useState: useStateEx, useMemo: useMemoEx } = React;
function ExpensesList({ expenses, onEdit, onDelete, onToggleStatus }) {
  const { Card, Badge, IconBtn, CatChip, Select, TextInput } = window.UI;
  const { Icon, fmtCLP, relDate, timeOnly, MES } = window.Helpers;
  ...
  const [fMonth, setFMonth] = useStateEx(`${today.getFullYear()}-${today.getMonth()}`);
  const [fCat,   setFCat]   = useStateEx("");
  const [fBank,  setFBank]  = useStateEx("");
  const [fMethod,setFMethod]= useStateEx("");
  const [fType,  setFType]  = useStateEx("");
    ...
    <div className="flex flex-col gap-4">
      <Card padding="p-3.5 md:p-4">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <Select value={fMonth} onChange={setFMonth} options={months} placeholder="" className="w-[150px]"/>
          <Select value={fCat} onChange={setFCat} placeholder="Categoría"
                  options={window.MOCK.CATEGORIES.map(c => ({ value: c.id, label: c.label }))} className="w-[150px]"/>
          <Select value={fBank} onChange={setFBank} placeholder="Banco"
                  options={window.MOCK.BANKS.map(b => ({ value: b.id, label: b.label }))} className="w-[150px]"/>
          <Select value={fMethod} onChange={setFMethod} placeholder="Medio"
                  options={window.MOCK.PAYMENT_METHODS.map(m => ({ value: m.id, label: m.label }))} className="w-[140px]"/>
          <Select value={fType} onChange={setFType} placeholder="Tipo"
                  options={[{value:"debito",label:"Débito"},{value:"credito",label:"Crédito"}]} className="w-[130px]"/>
          {activeFilters > 0 && (
      ...
      <div className="flex items-center justify-between text-[12.5px] text-[var(--muted)] px-1">
        ...
        <div>Ordenado por fecha desc.</div>
      </div>
      ...
      <Card padding="p-0" className="hidden md:block overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-[13px]">
            ...
            <tbody>
              {filtered.map(e => {
                const bank = window.MOCK.BANKS.find(b => b.id === e.bank);
                return (
                  <tr key={e.id} className="border-b last:border-b-0 border-[var(--line)] hover:bg-[var(--hover)] group">
                    ...
                    <td className="py-2 pl-4 pr-3 align-top">
                      <div className="flex items-center gap-0.5 justify-end opacity-50 group-hover:opacity-100 transition">
                        {e.status === "revisar" && (
                          <IconBtn name="check" label="Marcar como revisado" tone="ok"
          ...
          {filtered.length === 0 && (
            <div className="p-10 text-center text-[var(--muted)] text-[13px]">
