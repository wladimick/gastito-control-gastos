// TelegramSettings + BotChat (panel "Probar bot")
const { useState: useStateTg } = React;
function TelegramSettings({ botStatus, setBotStatus, lastBotMessage }) {
  const { Card, Badge } = window.UI;
  const { Icon, relDate, timeOnly } = window.Helpers;

  const [cfg, setCfg] = useStateTg({
  ...
  const [showToken, setShowToken] = useStateTg(false);
  const [testingConn, setTestingConn] = useStateTg(false);
  const [testResult, setTestResult] = useStateTg(null);
  const [saved, setSaved] = useStateTg(false);
        ...
        <Card padding="p-5 md:p-6">
          ...
          <div className="mt-6 pt-5 border-t border-[var(--line)] flex items-center justify-between flex-wrap gap-3">
            <button
              ...
              <Icon name={testingConn ? "refresh" : "link"} size={14} className={testingConn ? "animate-spin" : ""}/>
