// Componentes UI compartidos: Card, Badge, Button, Pill, BarChart simple
const { Icon, fmtCLP, fmtCLPshort } = window.Helpers;
...
function Badge({ children, tone = "neutral", className = "" }) {
  ...
  return <span className={`inline-flex items-center gap-1.5 text-[11px] font-medium px-2 py-[3px] rounded ${tones[tone]} ${className}`}>{children}</span>;
}

function IconBtn({ name, label, onClick, tone="ghost", title }) {
  const tones = {
...
function CatChip({ catId, size = "sm" }) {
  const cat = window.MOCK.CATEGORIES.find(c => c.id === catId) || window.MOCK.CATEGORIES[7];
  const s = size === "sm" ? "text-[11px] px-1.5 py-[2px]" : "text-xs px-2 py-1";
  return (
    <span className={`inline-flex items-center gap-1.5 rounded ${s}`} style={{ background: cat.color + "1A", color: "var(--ink)" }}>
...
function BarRow({ label, value, max, color = "var(--ink)", right }) {
...
function TextInput({ value, onChange, placeholder, type="text", icon, className="" }) {
  return (
...
window.UI = { Card, StatCard, Badge, IconBtn, CatChip, BarRow, Select, TextInput };
