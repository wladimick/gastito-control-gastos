...
export default function DashboardWithReimbursements(props) {
  ...
  if (!reimbursements.length || amount <= 0) return <DashboardFinancial {...props}/>

  return (
    <div className="flex flex-col gap-5">
      <Card padding="p-4 md:p-5" className="border-emerald-200 bg-emerald-50/50">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div className="flex items-start gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-emerald-100 text-emerald-700 grid place-items-center shrink-0"><Icon name="cash" size={17}/></div>
            <div className="min-w-0">
              ...
              <div className="text-[10.5px] text-emerald-900/65 mt-1 leading-relaxed">{reimbursements.length} rendición{reimbursements.length === 1 ? '' : 'es'} por cobrar{overdue ? ` · ${overdue} fuera de la fecha esperada` : ''}.</div>
            </div>
          </div>
          <div className="flex flex-col md:items-end gap-2 shrink-0">
            <div className="text-[10px] text-[var(--muted)] max-w-xs md:text-right">No se suma al saldo disponible hasta recibir la transferencia.</div>
            <button type="button" onClick={() => props.setView?.('reimbursements')} className="h-9 px-3 rounded-xl bg-[var(--ink)] text-[var(--bg)] text-[10.5px] font-semibold">Ver rendiciones</button>
          </div>
        </div>
      </Card>
