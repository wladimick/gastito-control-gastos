...
import { Icon, timeOnly } from '../lib/helpers'
import { CATEGORIES, BOT_CHAT } from '../data'

export default function BotChat({ open, onClose, onMessage }) {
  const [messages, setMessages] = useState(BOT_CHAT);
  ...
  const send = () => {
    ...
    setTimeout(() => {
      ...
      const cat = CATEGORIES.find(c => text.toLowerCase().includes(c.label.toLowerCase().split(" ")[0]));
      let reply;
      if (amount && cat) {
  ...
  return (
    <div className="fixed inset-0 z-40 pointer-events-none">
      <div className="absolute right-4 bottom-4 md:right-7 md:bottom-7 w-[min(380px,calc(100vw-2rem))] h-[min(560px,calc(100vh-7rem))] pointer-events-auto
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-2.5" style={{ background: "oklch(0.97 0.005 85)" }}>
          {messages.map((m, i) => (
