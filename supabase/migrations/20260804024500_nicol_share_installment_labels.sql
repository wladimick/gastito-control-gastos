    ...
    select
      ...
      jsonb_agg(
        jsonb_build_object(
          ...
          'description', case
            when item.movement_type = 'installment'
              and item.installment_current is not null
              and item.installment_total is not null
              then item.description || ' · Cuota ' || item.installment_current || '/' || item.installment_total
